...
<div class="content-box">
	<?php
if($_GET['subcat']=='add-home-content-therapist'){

	if(!empty($_GET['edit'])){

   $editId= $_GET['edit'];
   $query="SELECT * FROM therapist WHERE id=$editId";
   $res= $conn->query($query);
   $editData=$res->fetch_assoc();
   $contentSection= $editData['id'];
   $firstTitle= $editData['name'];
   $secondTitle= $editData['address'];
...
<?php }elseif (!empty($_GET['view'])) {?>
  ...
  $id= $_GET['view'];
   ...
   $backId=$viewData['id']-1;
    $firstTitle=$viewData['name'];
    $secondTitle=$viewData['address'];
	...
	<div class="row">
		<div class="col">
			<?php echo "<h4>Menu - ".$viewData['name']."</h4>"; ?>
		</div>
		<div class="col text-right">
		...
		<table class="table">
			<tr>
				...
				<th>Delete</th>
						...
						<?php
  $sql1="SELECT * FROM musics ORDER BY id DESC";
